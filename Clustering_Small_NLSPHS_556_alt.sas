...
ods pdf file="X:\xDATA\NLSPHS 2014\Analysis\AllClusters_alt.pdf";
%MACRO plotcan (N=);
...
/*The number of cluster was 15 with the optimal CCC. Thus, we will go with 6 clusters and take the data obtained
above (can15) */

proc freq data=can15;
tables cluster;
run;
...
PROC EXPORT DATA= WORK.SMALL_PEER_FINAL 
            OUTFILE= "X:\xDATA\NLSPHS 2014\Analysis\data\SMALL_PEER_FINAL_alt.dta" 
            DBMS=STATA REPLACE;
RUN;
